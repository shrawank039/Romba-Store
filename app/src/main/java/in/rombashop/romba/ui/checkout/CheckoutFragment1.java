...
import in.rombashop.romba.databinding.CheckoutFragment1Binding;
...
//    private AutoClearedValue<ProgressDialog> prgDialog;
    ...
    protected void initUIAndActions() {
        ...
        if (userViewModel.user != null) {
            ...
            if (userViewModel.user.userEmail != null && !userViewModel.user.userEmail.equals("")){
                binding.get().userEmailEditText.setText(this.userViewModel.user.userEmail);
            }
            ...
            if (userViewModel.user.shippingCountry != null && !userViewModel.user.shippingCountry.equals("")) {
                binding.get().countryTextView.setText(this.userViewModel.user.country.name);
            ...
            if (userViewModel.user.shippingCity != null && !userViewModel.user.shippingCity.equals("")) {
                binding.get().cityTextView.setText(this.userViewModel.user.city.name);
        ...
        binding.get().countrySelectionView.setOnClickListener(v ->
                navigationController.navigateToSearchActivityCategoryFragment(getActivity(), Constants.COUNTRY, Constants.NO_DATA, Constants.NO_DATA, userViewModel.countryId, userViewModel.cityId));
            ...
            if (listResource != null) {
                ...
                switch (listResource.status) {
                        ...
                        if (listResource.data != null) {
                         ...
                         //   Toast.makeText(CheckoutFragment1.this.getActivity(), CheckoutFragment1.this.getString(R.string.success), Toast.LENGTH_SHORT).show();
                            ...
                            if (CheckoutFragment1.this.getActivity() != null) {
                                ((CheckoutActivity) CheckoutFragment1.this.getActivity()).navigateFragment(((CheckoutActivity) CheckoutFragment1.this.getActivity()).binding, 2);
                            }
                    ...
                    case ERROR:
                        ...
                        if (CheckoutFragment1.this.getActivity() != null) {
                            ((CheckoutActivity) CheckoutFragment1.this.getActivity()).number = 1;
                        }
    ...
    boolean checkUserEmailEditTextIsEmpty(){
        return binding.get().userEmailEditText.getText().toString().isEmpty();
    }

    void checkUserShippingEmailAndBillingEmail(){
        if(binding.get().emailEditText.getText().toString().equals("")){
            binding.get().emailEditText.setText(binding.get().userEmailEditText.getText().toString());
        }

        if(binding.get().card2EmailEditText.getText().toString().equals("")){
            binding.get().card2EmailEditText.setText(binding.get().userEmailEditText.getText().toString());
        }
    }
