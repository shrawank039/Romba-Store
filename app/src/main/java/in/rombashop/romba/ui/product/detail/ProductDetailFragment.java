package in.rombashop.romba.ui.product.detail;
...
import in.rombashop.romba.ui.product.detail.adapter.ProductAttributeHeaderAdapter;
import in.rombashop.romba.ui.product.detail.adapter.ProductColorAdapter;
import in.rombashop.romba.ui.product.detail.adapter.ProductDetailListPagerAdapter;
import in.rombashop.romba.ui.product.detail.adapter.ProductDetailSpecsAdapter;
import in.rombashop.romba.ui.product.detail.adapter.ProductDetailTagAdapter;
import in.rombashop.romba.utils.AutoClearedValue;
import in.rombashop.romba.utils.BottomSheetDialogExpanded;
...
import in.rombashop.romba.utils.ViewAnimationUtil;
    ...
    protected void initAdapters() {
        ...
        ProductColorAdapter nvcolorAdapter = new ProductColorAdapter(dataBindingComponent,
                (productColor, seletedColorId, selectetColorValue) -> {
                    ...
                    productColorViewModel.proceededColorListData = ProductDetailFragment.this.processDataList(productColorViewModel.proceededColorListData);
                    ProductDetailFragment.this.replaceProductColorData(productColorViewModel.proceededColorListData);
                });
...
//                        productAttributeHeaderViewModel.attributeHeaderHashMap.clear();
    ...
    private void bindingDescData(String description) {
        if (description.isEmpty()) {
            binding.get().desc.setVisibility(View.GONE);
            binding.get().descUpDownImageView.setVisibility(View.GONE);
            binding.get().view96.setVisibility(View.GONE);
            binding.get().descTextView.setVisibility(View.GONE);
        } else {
            binding.get().desc.setVisibility(View.VISIBLE);
            binding.get().descUpDownImageView.setVisibility(View.VISIBLE);
            binding.get().view96.setVisibility(View.VISIBLE);
            binding.get().descTextView.setText(description);
        }
    }
    ...
    private void replaceProductDetailData(Product product) {
        ...
        if (productDetailViewModel.basketFlag.equals(Constants.ONE)) {
            binding.get().priceTextView.setText(Utils.format(Utils.round(Float.parseFloat(productDetailViewModel.price), 2)));

            bottomBoxLayoutBinding.get().priceTextView.setText(Utils.format(Utils.round(Float.parseFloat(productDetailViewModel.price), 2)));
